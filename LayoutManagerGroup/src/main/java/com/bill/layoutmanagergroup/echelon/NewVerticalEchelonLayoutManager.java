...
public class NewVerticalEchelonLayoutManager extends RecyclerView.LayoutManager {
    ...
    public NewVerticalEchelonLayoutManager(Context context) {
...
//            double maxOffset = 400.0 * pow;
            ...
            float layoutPercent = top * 1.0f / getVerticalSpace();
            ItemViewInfo info = new ItemViewInfo(top, scaleXY, positonOffset, layoutPercent);
...
//                info.setTop((int) (remainSpace + maxOffset));
        ...
        if (bottomItemPosition < mItemCount) {//当移动过了
            //总高度减去看得见的部分就是底部top值
